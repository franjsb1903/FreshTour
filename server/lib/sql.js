...
sql.planificacions.elementos = "SELECT *, 'Lugar turístico' AS tipo, (SELECT tipo_visita FROM fresh_tour.planificacions_lugares_turisticos plt WHERE plt.id_lugar_turistico = lt.id) as tipo_visita FROM fresh_tour.lugares_turisticos lt WHERE lt.id IN (SELECT id_lugar_turistico FROM fresh_tour.planificacions_lugares_turisticos plt WHERE plt.id_planificacion = $1) UNION all SELECT *, 'Monumento' AS tipo, (SELECT tipo_visita FROM fresh_tour.planificacions_monumentos pm WHERE pm.id_monumento = m.id) as tipo_visita FROM fresh_tour.monumentos m WHERE m.id IN (SELECT id_monumento FROM fresh_tour.planificacions_monumentos pm WHERE pm.id_planificacion = $1)"
